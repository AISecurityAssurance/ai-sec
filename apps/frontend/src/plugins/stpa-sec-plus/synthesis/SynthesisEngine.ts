...
import type { StandardizedAnalysis, SynthesisResult, AnalysisGap, CrossFrameworkInsight, AnalysisFramework } from '../types';
...
export class STPASecPlusSynthesisEngine {
  ...
  async synthesize(): Promise<SynthesisResult> {
    ...
    const recommendations = this.generateRecommendations(gaps, insights, analysesArray);
  ...
  private generateRecommendations(gaps: AnalysisGap[], insights: CrossFrameworkInsight[], analyses: StandardizedAnalysis[]): any[] {
  ...
  private calculateCompleteness(analyses: StandardizedAnalysis[]): number {
    ...
    requiredFrameworks.forEach(fw => {
      maxScore += 2;
      if (frameworks.has(fw as AnalysisFramework)) score += 2;
    });
    
    // Recommended frameworks
    recommendedFrameworks.forEach(fw => {
      maxScore += 1;
      if (frameworks.has(fw as AnalysisFramework)) score += 1;
    });
