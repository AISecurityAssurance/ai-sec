...
def _extract_stakeholder_harm(loss):
...
def _convert_to_system_state(description):
...
def _determine_hazard_category(description):
...
def _determine_affected_property(category):
...
def _generate_mapping_rationale(hazard_id, loss_id):
...
def _extract_success_perspective(stakeholder):
...
def _extract_failure_perspective(stakeholder):
...
def _map_stakeholder_losses(stakeholder_name):
...
def _generate_severity_perception(stakeholder_name):
    ...
    try:
        ...
        for loss in losses:
            ...
            """,
                ...
                json.dumps({
                    ...
                    "cascading_effects": (json.loads(loss['properties']) if isinstance(loss['properties'], str) else loss['properties']).get('impact_areas', []),
        ...
        for adv in adversaries:
            ...
            """,
                str(uuid4()),
                analysis_id,
                class_map.get(adv['primary_motivation'], 'opportunist'),
                json.dumps({
                    "sophistication": adv['technical_sophistication'],
                    "resources": adv['resources'],
                    "persistence": "long_term" if adv['technical_sophistication'] == 'high' else "opportunistic",
                    "primary_interest": adv['primary_motivation'],
                    "geographic_scope": "global" if adv['technical_sophistication'] == 'high' else "regional"
                }),
                json.dumps({
                    "interested_in": ["customer_assets", "transaction_capability", "customer_data"],
                    "value_perception": "high_value_target",
                    "historical_interest": "known_targeting" if adv['technical_sophistication'] == 'high' else "potential_target"
                })
            )
